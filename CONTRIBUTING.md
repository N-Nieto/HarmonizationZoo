...
| `in_uniharmony`, `also_implemented_in` | no | Legacy fields, kept for backward compatibility but no longer rendered anywhere on the site — toolbox membership is now driven entirely by `data/toolboxes.json` (see "Toolboxes" in `MAINTAINERS.md`). If you're adding a method that belongs in an existing toolbox, add its id to that toolbox's `methods` array in `data/toolboxes.json` instead of setting these. |
